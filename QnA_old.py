...
def retrieve_documents(query, embedded_docs, splits, top_n=3):
    ...
    #### calculating cosine similarity between query and all document embeddings ########
    similarities = cosine_similarity(query_embedding, embedded_docs)[0]

    ######### extracting the indices of n most similar documents ##########
...
def get_context_and_answer(splits, query, embedded_docs):
    ...
    input = tokenizer(query, context, return_tensors="pt")
    output = model(**input)
